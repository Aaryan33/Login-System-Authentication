...
export async function register(req, res){
    ...
    try {
        ...
        if(existUsername) return res.status(400).send({ error: "Username already exists" });
            ...
            .then(user => {
                bcrypt.compare(password, user.password)
                    .then(passwordCheck => {
                        ...
                        // create jwt token,  jwt - json web token -> defines a token for authentication
...
/** GET: http://localhost:8088/api/user/example123*/
...
/** PUT: http://localhost:8088/api/updateUser 
 * @param: {
  "header" : "<token>"
}
body: {
...
/** GET: http://localhost:8088/api/generateOTP*/
export async function generateOTP(req,res){
    req.app.locals.OTP = await otpGenerator.generate(6, { lowerCaseAlphabets: false, upperCaseAlphabets: false, specialChars: false})
    res.status(201).send({ code : req.app.locals.OTP });
}

/** GET: http://localhost:8088/api/verifyOTP*/
export async function verifyOTP(req,res){
    ...
    if(parseInt(req.app.locals.OTP) === parseInt(code)){
        ...
        return res.status(201).send({ msg : 'Verify Succeessfully!'});
    }
    return res.status(400).send({error : 'Invalid OTP'});
}

// successfully redirect user when OTP is valid
/** GET: http://localhost:8088/api/createResetSession*/
...
/** PUT: http://localhost:8088/api/resetPassword*/
